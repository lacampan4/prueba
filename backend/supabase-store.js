...
async function obtenerTotal(inicio, fin) {
  // 'estimated' usa las estadísticas del planificador de Postgres en vez de
  // recorrer/contar cada fila. Con 'exact', contar un rango de varios años
  // (cientos de miles o millones de filas) puede por sí solo agotar el
  // statement_timeout de Postgres (error 57014), aunque el resto de la
  // consulta sea rápido. El total exacto no es crítico aquí, solo se usa
  // para mostrar avance/paginación.
  const { count, error } = await supabase
    .from('sap_registros')
    .select('registro_hash', { count: 'estimated', head: true })
...
async function obtenerPagina(inicio, fin, limit, offset) {
  const { data, error, count } = await supabase
    .from('sap_registros')
    .select('registro_hash,sap_id,fecha_factura,datos_sap', { count: 'estimated' })
    .gte('fecha_factura', inicio)
    .lte('fecha_factura', fin)
...
async function contarRegistros(inicio, fin) {
  // 'estimated' evita que Postgres tenga que recorrer/contar todas las
  // filas del rango (ver nota en obtenerTotal). Es una aproximación, pero
  // aquí solo alimenta el porcentaje de avance, no el procesamiento real.
  const { count, error } = await supabase
    .from('sap_registros')
    .select('registro_hash', { count: 'estimated', head: true })
    .gte('fecha_factura', inicio)
...
async function recorrerRegistros(inicio, fin, onRows) {
  // Páginas más grandes = menos round-trips a Supabase cuando el rango
  // cubre varios años (antes 1000; con cientos de miles de filas eso son
  // muchísimas idas y vueltas). La paginación sigue siendo por cursor
  // (keyset), no por OFFSET, así que cada página cuesta lo mismo sin
  // importar cuán avanzada esté la lectura.
  const pageSize = 5000;

  // El total es solo informativo (progreso). Si el conteo llegara a fallar
  // o tardar demasiado, NO debe tumbar la lectura real de los datos: se
  // sigue paginando con total=0/desconocido en vez de propagar el error.
  let total = 0;
  try {
    total = await contarRegistros(inicio, fin);
  } catch (error) {
    console.warn('No se pudo estimar el total de registros (se continúa sin él):', error.message || error);
    total = 0;
  }

  let cursor = null;
  let leidos = 0;
  let tamanoActual = pageSize;
  while (true) {
    let data;
    try {
      data = await obtenerPaginaPorCursor(inicio, fin, tamanoActual, cursor);
    } catch (error) {
      // Código 57014 = "statement timeout" de Postgres. En vez de abortar
      // toda la lectura de años de datos por una sola página lenta,
      // reintentamos esa misma página pidiendo menos filas.
      const esTimeout = error && (error.code === '57014' || /statement timeout/i.test(error.message || ''));
      if (esTimeout && tamanoActual > 250) {
        tamanoActual = Math.max(250, Math.floor(tamanoActual / 4));
        console.warn(`Timeout leyendo Supabase, reintentando con páginas de ${tamanoActual} filas...`);
        continue;
      }
      throw error;
    }
    ...
    if (data.length < tamanoActual) break;
    // Si el tamaño se redujo por un timeout anterior, lo mantenemos así
    // durante el resto de la lectura para no volver a chocar con el límite.
  }
  return total || leidos;
}
