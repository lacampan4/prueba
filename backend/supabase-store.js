...
// Lectura robusta para rangos largos.
//
// No depende de cursor_key ni de una migración adicional en Supabase.
// Dividimos el rango solicitado en bloques mensuales y dentro de cada bloque
// usamos la clave compuesta existente (fecha_factura, registro_hash) para
// ordenar. Así PostgreSQL nunca tiene que ordenar/paginar cientos de miles de
// filas de varios años en una sola consulta.
...
async function obtenerPaginaMes(inicio, fin, limit, offset) {
  const { data, error } = await supabase
    .from('sap_registros')
...
async function recorrerRegistros(inicio, fin, onRows) {
  const pageSize = 2000;
  let bloqueInicio = primerDiaMes(inicio);
  const finReal = fin;
  let leidos = 0;
  let paginas = 0;
  let bloques = 0;

  while (bloqueInicio <= finReal) {
    const bloqueFin = [ultimoDiaMes(bloqueInicio), finReal].sort()[0];
    const rangoInicio = bloqueInicio > inicio ? bloqueInicio : inicio;
    const rangoFin = bloqueFin < finReal ? bloqueFin : finReal;
    let offset = 0;
    let paginasBloque = 0;

    while (true) {
      const data = await obtenerPaginaMes(rangoInicio, rangoFin, pageSize, offset);
      if (!data.length) break;

      paginas++;
      paginasBloque++;
      await onRows(data.map(row => row.datos_sap), leidos, null, {
        pagina: paginas,
        paginaBloque: paginasBloque,
        tamanoPagina: data.length,
        bloqueInicio: rangoInicio,
        bloqueFin: rangoFin,
        bloque: bloques + 1
      });
    ...
    bloques++;
    bloqueInicio = sumarMes(bloqueInicio);
  }
