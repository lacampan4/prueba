...
async function obtenerPagina(inicio, fin, limit, offset, opciones = {}) {
  // Por defecto NO pedimos count=exact. Ese COUNT obliga a PostgreSQL/Supabase
  // a contar potencialmente cientos de miles de filas y puede provocar
  // 'canceling statement due to statement timeout'.
  const contar = opciones.contar === true;
  const query = supabase
    .from('sap_registros')
    .select('registro_hash,sap_id,fecha_factura,datos_sap', contar ? { count: 'exact' } : undefined)
    .gte('fecha_factura', inicio)
    .lte('fecha_factura', fin)
  ...
  if (error) throw error;
  return { data: data || [], total: contar ? (count || 0) : null };
}
...
async function recorrerRegistros(inicio, fin, onRows) {
  ...
  let total = 0;
  while (true) {
    // Lectura ligera: sin COUNT global. El total real se obtiene sumando las
    // páginas que efectivamente se procesan.
    const page = await obtenerPagina(inicio, fin, pageSize, offset, { contar: false });
    if (!page.data.length) break;
    await onRows(page.data.map(row => row.datos_sap), offset, null);
