...
app.get('/auth/me',requiereAuth,async (req,res)=>{
  try {
    const u=await getUserByUsername(req.usuarioAuth.sub);
    if(!u || u.activo === false) return res.status(401).json({ok:false,message:'Usuario desactivado o no encontrado.'});
