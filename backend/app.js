...
async function obtenerFilas(inicio, fin) {
  ...
  const { data, total } = await obtenerPagina(inicio, fin, 1000, 0, { contar: false });
...
app.get('/facturacion', requiereAuth, async (req, res) => {
  ...
  try {
    ...
    const { data, total } = await obtenerPagina(fecha_inicio, fecha_fin, limit, offset, { contar: true });
