...
// Cada consulta iniciada por un usuario tiene su propio estado.
// Así un usuario no ve ni hereda el progreso de otro usuario.
// La descarga real puede compartir una misma consulta SOLO cuando el
// rango es exactamente el mismo, evitando golpear SAP dos veces por error.
const syncJobs = new Map();
const syncJobsByRange = new Map();

function crearSyncJob(inicio, fin) {
  ...
  syncJobsByRange.set(claveRango(inicio, fin), jobId);
  return job;
}

function obtenerSyncJob(jobId) {
  return jobId ? syncJobs.get(String(jobId)) : null;
}

function limpiarSyncJob(jobId, inicio, fin) {
  syncJobsByRange.delete(claveRango(inicio, fin));
  // Conservamos el resultado unos minutos para que el navegador pueda
  // consultar el estado final incluso si llega una petición tardía.
  setTimeout(() => syncJobs.delete(jobId), 10 * 60 * 1000).unref?.();
}
...
console.log('SAP CONCURRENCIA MAXIMA:', SAP_MAX_CONCURRENCIA);
...
function actualizarActividad(estado = syncState) {
...
app.get('/sync-status', requiereAuth, (req, res) => {
  ...
  const job = obtenerSyncJob(jobId);

  if (jobId && !job) {
    return res.status(404).json({
      ok: false,
      error: 'La consulta SAP no existe o ya expiró.',
      jobId
    });
  }

  // Compatibilidad: sin jobId devolvemos un resumen global, pero los
  // botones del dashboard siempre consultan su jobId individual.
  const estado = job || syncState;
  ...
  res.json({
    ...
    sincronizacion: {
      ...
      skipActual: estado.skipActual,
      paginasProcesadas: estado.paginasProcesadas,
      paginaSize: SAP_PAGE_SIZE,
      registrosSAP: estado.registrosSAP,
      registrosProcesados: estado.registrosProcesados,
      registrosTotal: estado.registrosTotal,
      iniciadoEn: estado.iniciadoEn,
      terminadoEn: estado.terminadoEn,
...
//
// El sistema está preparado para hasta 15 usuarios/consultas SAP
// simultáneas. Este límite cuenta peticiones a SAP, no usuarios: los
// 15 trabajos pueden existir y cada uno conserva su propio jobId/progreso.
// Si SAP necesita un límite menor, se puede reducir mediante
// SAP_MAX_CONCURRENCIA sin modificar el código.
//
...
// llamadas simultáneas a SAP (SAP_MAX_CONCURRENCIA, por defecto 15) y,
...
const SAP_MAX_CONCURRENCIA = Math.max(1, parseInt(process.env.SAP_MAX_CONCURRENCIA, 10) || 15);
...
async function descargarRangoDesdeSAP(inicio, fin, opciones = {}) {
  const actualizarEstado = Boolean(opciones && opciones.actualizarEstado);
  const estadoSync = (opciones && opciones.syncState) || syncState;
  ...
  for (const parte of partes) {
    ...
    while (true) {
      ...
      if (actualizarEstado) {
        estadoSync.paginaActual = paginaGlobal;
        estadoSync.skipActual = skip;
        actualizarActividad();
      }
      ...
      if (actualizarEstado && paginaMes === 1) {
        ...
        if (Number.isFinite(countMes) && countMes >= 0) {
          estadoSync.registrosTotal += countMes;
        }
      }
      ...
      if (actualizarEstado) {
        estadoSync.registrosSAP += cantidad;
        estadoSync.registrosProcesados += registros.length;
        estadoSync.paginasProcesadas = paginaGlobal;
        actualizarActividad();
      }
...
function iniciarDescarga(inicio, fin, opciones) {
  ...
  const promesa = descargarRangoDesdeSAP(inicio, fin, opciones || {})
    .then(resultado => {
...
app.get('/sync-sap', requiereAuth, async (req, res) => {
  ...
  // Si el rango ya está fresco, no se vuelve a consultar SAP.
  if (!forzar) {
    const entry = obtenerEntradaCache(inicio, fin);
    if (entry) {
  ...
  // Si otro usuario ya está consultando EXACTAMENTE el mismo rango,
  // ambos pueden observar esa misma descarga. Esto evita duplicarla.
  // Un rango diferente crea su propio job y no recibe un 409 por la
  // consulta que está haciendo otro usuario.
  if (syncJobsByRange.has(key)) {
    const existingJobId = syncJobsByRange.get(key);
    const existingJob = obtenerSyncJob(existingJobId);
    if (existingJob && existingJob.ejecutando) {
      return res.status(202).json({
        ok: true,
        compartida: true,
        mensaje: 'Este mismo rango ya se está consultando en SAP. Se comparte la consulta para no duplicarla.',
        jobId: existingJobId,
        fechas: { inicio, fin },
        paginaSize: SAP_PAGE_SIZE,
        estado: 'procesando',
        consultarEstado: `/sync-status?jobId=${encodeURIComponent(existingJobId)}`
      });
    }
  }

  // Aunque el mismo rango no esté en syncJobsByRange, iniciarDescarga()
  // también protege contra duplicados mediante descargasEnCurso.
  // Para una consulta nueva creamos un estado independiente.
  const job = crearSyncJob(inicio, fin);

  console.log('===========================================');
  console.log('SYNC SAP INICIADA EN SEGUNDO PLANO');
  console.log(`JOB ${job.jobId} | ${inicio} → ${fin}`);
  console.log('===========================================');

  iniciarDescarga(inicio, fin, {
    actualizarEstado: true,
    syncState: job
  })
    .then(resultado => {
      job.ejecutando = false;
      job.estado = 'completado';
      job.terminadoEn = new Date().toISOString();
      actualizarActividad(job);
      ...
      limpiarSyncJob(job.jobId, inicio, fin);
    })
    .catch(error => {
      console.error('===========================================');
      console.error('✗ ERROR CONSULTANDO SAP');
      console.error(`✗ JOB ${job.jobId}`);
      console.error(error);
      console.error('===========================================');
      ...
      // Permitimos volver a lanzar el mismo rango después del error.
      syncJobsByRange.delete(key);
    });

  return res.status(202).json({
    ok: true,
    mensaje: 'Consulta a SAP iniciada en segundo plano para este usuario',
    jobId: job.jobId,
    fechas: { inicio, fin },
    paginaSize: SAP_PAGE_SIZE,
    estado: 'procesando',
    consultarEstado: `/sync-status?jobId=${encodeURIComponent(job.jobId)}`
  });
});
