...
// Una sola construcción por rango. Si el navegador reintenta por un timeout
// de red, todos los intentos esperan el mismo trabajo en vez de reconstruir
// el dataset completo en paralelo.
const datasetBuildPromises = new Map();
...
async function obtenerDataset(inicio, fin, opciones) {
  const key = claveDatasetBuild(inicio, fin);
  const existente = datasetBuildPromises.get(key);
  ...
  const promesa = (async () => {
    ...
    await recorrerRegistros(inicio, fin, async (rows, offset, total) => {
      actualizarEstadoDataset(inicio, fin, {
        etapa: 'leyendo_supabase',
        totalRegistros: Number(total || 0),
        registrosLeidos: Math.min(Number(total || 0), Number(offset || 0) + rows.length),
    ...
    actualizarEstadoDataset(inicio, fin, {
      etapa: 'finalizando',
      registrosProcesados: totalRegistros,
      registrosLeidos: estado.totalRegistros || totalRegistros
    });
    ...
    finalizarEstadoDataset(inicio, fin, {
      estado: 'completado',
      etapa: 'completado',
      totalRegistros: estado.totalRegistros || totalRegistros,
      registrosLeidos: estado.totalRegistros || totalRegistros,
      registrosProcesados: totalRegistros
    });

      return { dataset, totalRegistros, deCache: resultado.yaGuardado };
    } catch (error) {
      finalizarEstadoDataset(inicio, fin, {
        estado: 'error',
        etapa: 'error',
  ...
  datasetBuildPromises.set(key, promesa);
  promesa.finally(() => {
    if (datasetBuildPromises.get(key) === promesa) datasetBuildPromises.delete(key);
  });
  return promesa;
}
