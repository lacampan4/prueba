...
// Caché del dataset ya agregado. La clave es el rango YYYY-MM-DD|YYYY-MM-DD.
// Se invalida después de cada sincronización SAP.
...
async function ejecutarSincronizacion(
  ...
  try {
    ...
    // Preparamos el dataset en Render antes de marcar la sincronización
    // como completada. Así, cuando el navegador reciba "completado",
    // ya no tiene que descargar/procesar miles de filas de facturación.
    try {
      const cacheKey = `${inicio}|${fin}`;
      ...
      guardarDatasetCache(cacheKey, buildDatasetFromFacturacion(result.rows));
      console.log(`✓ Dataset comercial preparado en Render: ${result.rows.length} filas`);
    } catch (datasetError) {
      console.error('No se pudo preconstruir el dataset comercial:', datasetError.message);
      // No hacemos fallar la sincronización SAP solo por el caché. El endpoint
      // /dataset podrá construirlo bajo demanda.
    }
...
app.get(
  ...
  async (req, res) => {
    ...
    // Cualquier sincronización nueva invalida los datasets anteriores,
    // porque sus rangos pueden solaparse con los datos recién importados.
    datasetCache.clear();
...
app.get('/dataset', async (req, res) => {
  ...
  const cacheKey = `${fecha_inicio}|${fecha_fin}`;
  ...
  try {
    ...
    const dataset = buildDatasetFromFacturacion(result.rows);
    guardarDatasetCache(cacheKey, dataset);
    ...
    return res.json({
      ...
      ...dataset,
      fechas: { inicio: fecha_inicio, fin: fecha_fin },
      filasProcesadas: result.rows.length
    });
  } catch (error) {
