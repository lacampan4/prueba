...
async function obtenerDataset(inicio, fin, opciones) {
  ...
  try {
    ...
    return { dataset, margen, totalRegistros, deCache: resultado.yaGuardado };
  } catch (error) {
    finalizarEstadoDataset(inicio, fin, {
      estado: 'error',
      etapa: 'error',
...
app.get('/dataset', requiereAuth, async (req, res) => {
  ...
  try {
    const { dataset, margen, totalRegistros, deCache } = await obtenerDataset(fecha_inicio, fecha_fin);
