...
// entorno en Render: SAP_SERVICE_URL, SAP_HOST, SAP_USER, SAP_PASS, etc.)
const SAP_BASE_URL = process.env.SAP_SERVICE_URL || process.env.SAP_BASE_URL || '';
...
app.get('/diagnostico-sap', async (req, res) => {
...
// ============================================================
function diagnosticoTLS(host, port, servername, etiqueta) {
  return new Promise((resolve) => {
    ...
    socket.once('secureConnect', () => {
      ...
      finish({
        ...
        protocoloTLS: socket.getProtocol?.() || null,
        autorizado: socket.authorized,
        errorAutorizacion: socket.authorizationError || null,
        certificadoCN: cert.subject?.CN || null,
        emisorCN: cert.issuer?.CN || null,
        ms: Date.now() - started
      });
    });

    socket.once('timeout', () => finish({
      ok: false, etiqueta, host, port, servername: servername || null,
      codigo: 'TIMEOUT', mensaje: 'Tiempo agotado durante la conexion TLS',
      ms: Date.now() - started
    }));
...
async function ejecutarDiagnosticoSAP() {
  console.log('===========================================');
  console.log('DIAGNOSTICO TEMPORAL SAP / TLS');
  console.log('===========================================');

  try {
    const parsed = new URL(SAP_BASE_URL);
    console.log('URL SAP valida:', parsed.protocol + '//' + parsed.hostname + ':' + (parsed.port || (parsed.protocol === 'https:' ? '443' : '80')) + parsed.pathname);
    console.log('Host SAP configurado:', SAP_HOST || '(vacio)');

    const port = Number(parsed.port || 443);
    const pruebas = await Promise.all([
      diagnosticoTLS(parsed.hostname, port, undefined, 'IP/host de la URL sin SNI forzado'),
      diagnosticoTLS(parsed.hostname, port, SAP_HOST || parsed.hostname, 'IP/host de la URL con SNI = SAP_HOST')
    ]);

    for (const r of pruebas) console.log('RESULTADO TLS:', JSON.stringify(r));
  } catch (err) {
    console.log('RESULTADO DIAGNOSTICO:', JSON.stringify({
      ok: false,
      codigo: err.code || null,
      mensaje: err.message || String(err)
    }));
  ...
  console.log('===========================================');
  console.log('FIN DIAGNOSTICO TEMPORAL SAP / TLS');
  console.log('===========================================');
}
...
// DIAGNOSTICO TEMPORAL: una ejecucion al arrancar.
setTimeout(() => ejecutarDiagnosticoSAP().catch(err => console.log('Diagnostico SAP fallo:', err.message)), 3000);
