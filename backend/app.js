...
// En Render la memoria disponible puede ser limitada. Evitamos que dos
// rangos DISTINTOS de SAP se descarguen y agreguen al mismo tiempo.
// Para el mismo rango seguimos compartiendo la misma promesa mediante
// descargasEnCurso. Así solo existe un dataset pesado en construcción a la vez.
let descargaGlobalEnCurso = null;
...
function iniciarDescarga(usuario, inicio, fin, opciones) {
  ...
  const ejecutar = async () => {
    // Esperar a cualquier descarga de otro rango para no duplicar el pico
    // de memoria. Si la descarga global terminó, continuamos normalmente.
    if (descargaGlobalEnCurso) await descargaGlobalEnCurso;
    ...
    descargaGlobalEnCurso = trabajo;
    try {
      return await trabajo;
    } finally {
      if (descargaGlobalEnCurso === trabajo) descargaGlobalEnCurso = null;
    }
  };
