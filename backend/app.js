...
// PostgreSQL se utiliza exclusivamente para usuarios, roles,
// permisos y autenticación.
//
// Se restaura la configuración de conexión SAP del proyecto
// que funcionaba antes de agregar usuarios/PostgreSQL.
...
// Los dashboards que forman parte de "Hoja de Ruta 6" usan el mismo flujo SAP/Supabase.
...
const PAGINAS_PERMITIDAS = [
  ...
  'panorama-produccion-diaria','panorama-portafolio','planeacion-nogales','costos-produccion','analisis-inventario','panorama-compras','panorama-margen','surtidos-sedes','usuarios'
];
const PERMISOS_POR_ROL = {
  admin: PAGINAS_PERMITIDAS,
  gerencia: PAGINAS_PERMITIDAS.filter(p => p !== 'usuarios'),
  comercio: ['panorama-comercial','hoja-asesor','hoja-ruta-cliente','hoja-sede','panorama-portafolio','panorama-compras','panorama-margen','surtidos-sedes'],
  produccion: ['panorama-produccion','hoja-despacho','hoja-sede','panorama-produccion-diaria','planeacion-nogales','costos-produccion','analisis-inventario','surtidos-sedes']
};
...
// ============================================================
// ALMACENAMIENTO HISTÓRICO DE SAP EN SUPABASE
// ============================================================
// Supabase es el almacenamiento permanente. La cobertura se controla por
// día: si un día ya fue consultado y completado, nunca se vuelve a pedir
// a SAP automáticamente. Los registros originales completos se guardan
// en jsonb y no se eliminan.

const descargasEnCurso = new Map();

// Cache en memoria del dataset ya procesado. Los registros crudos siguen
// siendo permanentes en Supabase; este cache solo evita reconstruir cientos
// de miles de filas cada vez que un usuario abre/refresca el mismo rango.
const datasetCache = new Map();
const DATASET_CACHE_MAX = 4;
...
console.log('===========================================');
console.log('BACKEND LA CAMPANA (SAP + historico permanente en Supabase)');
console.log('===========================================');
console.log('SAP:', SAP_BASE_URL);
console.log('SAP HOST:', SAP_HOST);
console.log('SAP PAGE SIZE:', SAP_PAGE_SIZE);
console.log('HISTORIAL SAP: Supabase permanente (sin borrado)');
...
app.get('/', (req, res) => {
  res.json({
    ...
    mensaje: 'Servidor funcionando correctamente (SAP + historico permanente en Supabase)',
    rutas: {
      health: '/health',
...
app.get('/health', (req, res) => {
  res.json({
    ok: true,
    estado: 'ok',
    fuenteDatos: 'SAP + Supabase historico permanente',
    sincronizacion: {
...
app.get('/diagnostico-sap', requiereAuth, requiereAdmin, async (req, res) => {
  await ejecutarDiagnosticoSAP();
  res.json({ ok: true, mensaje: 'Diagnostico ejecutado. Revisa los Logs de Render.' });
});
...
async function consultarSAPPaginaInterna(inicio, fin, skip, top) {
  ...
  try {
    const response = await axios.get(SAP_BASE_URL, {
      ...
      httpsAgent: new https.Agent({ rejectUnauthorized: SAP_TLS_REJECT_UNAUTHORIZED }),
      timeout: 600000,
...
async function descargarRangoDesdeSAP(inicio, fin, actualizarEstado) {
  ...
  for (const parte of partes) {
    ...
    while (true) {
      ...
      // IMPORTANTE: antes, si SAP respondía HTTP 200 pero con un cuerpo que
      // no tenía la forma esperada {d:{results:[...]}} (sesión OData vencida,
      // respuesta de error disfrazada de 200, etc.), `data?.d?.results || []`
      // lo convertía silenciosamente en "0 registros" y el mes se marcaba
      // como cubierto/completado para siempre, sin datos reales y sin volver
      // a reintentarse jamás. Esto es lo que causaba que, al pedir un rango
      // de varios meses, solo el/los mes(es) que sí habían sincronizado bien
      // antes mostraran información y el resto quedara vacío sin error visible.
      if (!data || typeof data !== 'object' || !data.d || !Array.isArray(data.d.results)) {
        throw new Error(
          `SAP devolvió una respuesta con formato inesperado para ${parte.inicio} → ${parte.fin} ` +
          `(página ${paginaMes}, skip ${skip}). No se marca ese período como sincronizado.`
        );
      }
      ...
      if (actualizarEstado && paginaMes === 1) {
        ...
        if (Number.isFinite(countMes) && countMes >= 0) syncState.registrosTotal += countMes;
      }

      console.log(`SAP devolvió ${cantidad} registros${Number.isFinite(Number(data?.d?.__count)) ? ` de ${data.d.__count} en este mes` : ''}`);
      if (cantidad === 0) break;

      // IMPORTANTE: guardamos el objeto original completo que devuelve SAP.
      await guardarRegistrosSAP(resultados, parte.inicio, parte.fin);
      registrosParte += cantidad;
      totalNuevos += cantidad;

      if (actualizarEstado) {
        syncState.registrosSAP += cantidad;
        syncState.registrosProcesados += cantidad;
    ...
    // Solo marcamos cobertura después de completar TODA la paginación de la
    // parte. Si SAP falla antes, ese período queda pendiente y se reintentará.
    await marcarCobertura(parte.inicio, parte.fin, registrosParte);
    console.log(`✓ Cobertura guardada en Supabase: ${parte.inicio} → ${parte.fin}`);
  }

  return { totalRegistros: totalNuevos };
}

async function asegurarRangoEnSupabase(inicio, fin, opciones = {}) {
  const cubiertos = await obtenerDiasCubiertos(inicio, fin);
  const faltantes = construirRangosFaltantes(inicio, fin, cubiertos);

  if (!faltantes.length) {
    return { yaGuardado: true, faltantes: [], registrosNuevos: 0 };
  }

  let registrosNuevos = 0;
  for (const rango of faltantes) {
    const resultado = await descargarRangoDesdeSAP(rango.inicio, rango.fin, opciones.actualizarEstado);
    registrosNuevos += resultado.totalRegistros;
  }

  return { yaGuardado: false, faltantes, registrosNuevos };
}

function iniciarDescarga(inicio, fin, opciones = {}) {
  const key = claveRango(inicio, fin);
  if (descargasEnCurso.has(key)) return descargasEnCurso.get(key);

  const promesa = asegurarRangoEnSupabase(inicio, fin, opciones)
    .finally(() => descargasEnCurso.delete(key));
  descargasEnCurso.set(key, promesa);
  return promesa;
}

async function obtenerFilas(inicio, fin) {
  const resultado = await iniciarDescarga(inicio, fin);
  const { data } = await obtenerPagina(inicio, fin, 1000);
  const filas = data.map(row => mapSAPRecord(row.datos_sap));
  return { filas, totalRegistros: filas.length, filasCrudasTruncadas: true, deCache: resultado.yaGuardado };
}

async function obtenerDataset(inicio, fin, opciones) {
  const cacheKey = claveRango(inicio, fin);
  const cached = obtenerDatasetCache(cacheKey);
  if (cached) return { ...cached, deCache: true, datasetCache: true };

  const resultado = await iniciarDescarga(inicio, fin, opciones || {});
  const acc = crearAcumulador();
  let totalRegistros = 0;
  await recorrerRegistros(inicio, fin, async (rows) => {
    for (const raw of rows) {
      const r = mapSAPRecord(raw);
      if (!r.sap_id) continue;
      acumularFila(acc, r);
      totalRegistros++;
    }
  });
  const value = { dataset: finalizarDataset(acc), totalRegistros, deCache: resultado.yaGuardado };
  guardarDatasetCache(cacheKey, value);
  return value;
}

async function obtenerAgrupado(inicio, fin, nombre, opciones = {}) {
  const resultado = await iniciarDescarga(inicio, fin, opciones);
  const agrupador = crearAgrupador(DEFINICIONES_AGRUPADORES[nombre]);
  let totalRegistros = 0;
  await recorrerRegistros(inicio, fin, async (rows) => {
    for (const raw of rows) {
      const r = mapSAPRecord(raw);
      if (!r.sap_id) continue;
      agregarFilaAgrupador(agrupador, r);
      totalRegistros++;
    }
  });
  return { grupos: finalizarAgrupador(agrupador), deCache: resultado.yaGuardado, totalRegistros };
}
...
// ============================================================
// SYNC-SAP: consulta SAP solo para las fechas que todavía no están cubiertas
// y guarda los registros completos de forma permanente en Supabase.
// /dataset reconstruye el rango solicitado directamente desde el histórico.
// ============================================================
...
app.get('/sync-sap', requiereAuth, async (req, res) => {
  ...
  if (!forzar) {
    const estado = await estadoRango(inicio, fin);
    if (estado.completo) {
      return res.status(200).json({
        ok: true,
        yaSincronizado: true,
        mensaje: 'Este rango ya está guardado en Supabase. No se volvió a consultar SAP.',
        fechas: { inicio, fin },
        diasGuardados: estado.cubiertos,
  ...
  iniciarDescarga(inicio, fin, { actualizarEstado: true, forzar })
    .then(resultado => {
      syncState.ejecutando = false;
      syncState.estado = 'completado';
      syncState.terminadoEn = new Date().toISOString();
      invalidarDatasetCache();
      actualizarActividad();

      console.log('===========================================');
      console.log('✓ CONSULTA SAP COMPLETADA');
      console.log(`✓ Registros recibidos de SAP y almacenados/intentados en Supabase: ${resultado.totalRegistros}`);
      console.log('===========================================');
    })
    .catch(error => {
      console.error('===========================================');
...
app.get('/facturacion', requiereAuth, async (req, res) => {
  ...
  try {
    await iniciarDescarga(fecha_inicio, fecha_fin);
    const { data, total } = await obtenerPagina(fecha_inicio, fecha_fin, limit, offset);
    const pagina = data.map(row => mapSAPRecord(row.datos_sap));
...
app.use((error, req, res, next) => {
  ...
  // El detalle completo del error (rutas internas, mensajes de SAP/Postgres,
