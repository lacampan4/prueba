...
// ============================================================
// CARGA DE EXCEL · PLANEACIÓN NOGALES
// ============================================================
// Recibe el Excel desde el navegador, lo interpreta en Render y devuelve
// exactamente el formato que espera planeacion-nogales.html.
// Acepta dos formatos:
// 1) Matriz: Código/Artículo/Grupo + una columna por mes.
// 2) Detalle: Código/Artículo/Grupo + Fecha + Kilos.
// También reconoce nombres de columnas comunes en español.
...
function mesDesdeCabecera(v) {
  const raw = String(v == null ? '' : v).trim();
  if (!raw) return null;
  const norm = normalizarTextoExcel(raw);
  ...
  let m = norm.match(/^(20\d{2})\s+([a-z]+)/);
  if (!m) m = norm.match(/^([a-z]+)\s+(20\d{2})$/) && [null, RegExp.$2, RegExp.$1];
  if (m) {
...
function fechaAñoMes(v) {
  if (v instanceof Date && !isNaN(v)) {
    return `${v.getFullYear()}-${String(v.getMonth()+1).padStart(2,'0')}`;
  }
  const s = String(v == null ? '' : v).trim();
  const iso = s.match(/^(20\d{2})[-\/](0?[1-9]|1[0-2])(?:[-\/]\d{1,2})?/);
  if (iso) return `${iso[1]}-${String(iso[2]).padStart(2,'0')}`;
  const dmy = s.match(/^(0?[1-9]|[12]\d|3[01])[-\/](0?[1-9]|1[0-2])[-\/](20\d{2})/);
  if (dmy) return `${dmy[3]}-${String(dmy[2]).padStart(2,'0')}`;
  return mesDesdeCabecera(s);
}

function buscarColumna(headers, candidatos) {
  const hs = headers.map(normalizarTextoExcel);
  for (const c of candidatos) {
    const n = normalizarTextoExcel(c);
    const i = hs.findIndex(h => h === n);
    if (i >= 0) return i;
  }
  for (const c of candidatos) {
    const n = normalizarTextoExcel(c);
    const i = hs.findIndex(h => h.includes(n));
    if (i >= 0) return i;
  }
  return -1;
}

app.post('/excel/planeacion-nogales', excelUpload.single('file'), (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ ok:false, error:'No se recibió ningún archivo Excel.' });

    // Algunos reportes de SAP llegan con extensión .xls pero en realidad son
    // archivos TSV de texto UTF-16 LE. Este formato es precisamente el que usa
    // "Hoja de Ruta No. 6". Si detectamos ese formato, NO pasamos los 130+ MB
    // por XLSX.read(): los procesamos como texto y evitamos el error de formato
    // y el consumo de memoria innecesario.
    let rows;
    let sheetName = req.file.originalname;
    const buf = req.file.buffer;
    const isUtf16 = buf.length >= 4 && ((buf[0] === 0xFF && buf[1] === 0xFE) || (buf[0] === 0xFE && buf[1] === 0xFF));
    const looksText = isUtf16 || (buf.length >= 200 && buf.slice(0, 200).includes(0));

    if (looksText) {
      let text;
      if (buf[0] === 0xFF && buf[1] === 0xFE) text = buf.toString('utf16le');
      else if (buf[0] === 0xFE && buf[1] === 0xFF) {
        // UTF-16 BE: swap bytes before decoding as LE.
        const swapped = Buffer.allocUnsafe(buf.length - (buf.length % 2));
        for (let i = 0; i + 1 < swapped.length; i += 2) { swapped[i] = buf[i + 1]; swapped[i + 1] = buf[i]; }
        text = swapped.toString('utf16le');
      } else text = buf.toString('utf8');
      text = text.replace(/^\uFEFF/, '');
      const lines = text.split(/\r?\n/);
      rows = lines.filter(line => line.length).map(line => line.split('\t'));
      sheetName = 'Texto TSV';
    } else {
    ...
    if (!rows.length) throw new Error('La primera hoja del Excel está vacía.');

    // Busca la primera fila que parezca encabezado.
    let headerRow = 0;
    for (let i=0; i<Math.min(rows.length,15); i++) {
      const joined = rows[i].map(normalizarTextoExcel).join(' | ');
      if (/codigo|referencia|articulo|producto/.test(joined) && /grupo|descripcion|kilo|kg|enero|feb|mar|abr|may|jun|jul|ago|sep|oct|nov|dic|20\d{2}/.test(joined)) {
        headerRow = i; break;
      }
    }
    const headers = rows[headerRow].map(v => String(v == null ? '' : v).trim());
    const dataRows = rows.slice(headerRow + 1).filter(r => r.some(v => v !== '' && v != null));
    const hNorm = headers.map(normalizarTextoExcel);

    const iCodigo = buscarColumna(headers, ['codigo','codigo articulo','codigo_articulo','referencia','ref','sku','cod']);
    const iDesc = buscarColumna(headers, ['descripcion','articulo','producto','nombre articulo','material']);
    const iGrupo = buscarColumna(headers, ['grupo','grupo articulo','categoria','categoría']);
    const iFecha = buscarColumna(headers, ['fecha','fecha factura','fecha_factura','mes']);
    const iKg = buscarColumna(headers, ['kilos','kg','kilogramos','cantidad kilos','cantidad']);

    // Detecta columnas mensuales en el encabezado.
    const monthCols = [];
    headers.forEach((h,i) => {
      const ym = mesDesdeCabecera(h);
      if (ym) monthCols.push({i, ym});
    });

    if (iCodigo < 0) throw new Error('No encontré una columna de Código/Referencia/Artículo. Revisa los encabezados del Excel.');
    if (iDesc < 0) throw new Error('No encontré una columna de Descripción/Artículo.');
    if (iGrupo < 0) throw new Error('No encontré una columna de Grupo.');

    const articles = {};
    const monthsSet = new Set();

    function ensure(cod, desc, grp) {
      const key = String(cod || '').trim();
      if (!key) return null;
      if (!articles[key]) articles[key] = { d:String(desc || key).trim(), g:String(grp || '(s/g)').trim() || '(s/g)', m:{} };
      if (!articles[key].d && desc) articles[key].d = String(desc).trim();
      if ((!articles[key].g || articles[key].g === '(s/g)') && grp) articles[key].g = String(grp).trim();
      return articles[key];
    }

    if (monthCols.length) {
      // Formato matriz: una columna por mes.
      for (const row of dataRows) {
        const cod = row[iCodigo];
        const desc = row[iDesc];
        const grp = row[iGrupo];
        const a = ensure(cod, desc, grp);
        if (!a) continue;
        const descTxt = String(desc || '');
        if (/segunda/i.test(descTxt)) continue;
        for (const mc of monthCols) {
          const ym = mc.ym;
          const kg = numeroExcel(row[mc.i]);
          monthsSet.add(ym);
          a.m[ym] = (a.m[ym] || 0) + kg;
        }
      }
    } else {
      // Formato detalle: Fecha + Kilos por fila.
      if (iFecha < 0 || iKg < 0) {
        throw new Error('No encontré columnas mensuales ni tampoco Fecha + Kilos. El Excel debe tener una de esas dos estructuras.');
      }
      for (const row of dataRows) {
        const cod = row[iCodigo];
        const desc = row[iDesc];
        const grp = row[iGrupo];
        const ym = fechaAñoMes(row[iFecha]);
        const a = ensure(cod, desc, grp);
        if (!a || !ym) continue;
        if (/segunda/i.test(String(desc || ''))) continue;
        monthsSet.add(ym);
        a.m[ym] = (a.m[ym] || 0) + numeroExcel(row[iKg]);
      }
    }

    const months = [...monthsSet].sort();
    const outArticles = {};
    for (const [cod,a] of Object.entries(articles)) {
      const m = months.map(ym => Math.round((a.m[ym] || 0) * 100) / 100);
      if (m.some(v => v !== 0)) outArticles[cod] = { d:a.d, g:a.g, m };
    }

    if (!months.length) throw new Error('No encontré meses con datos en el Excel.');
    if (!Object.keys(outArticles).length) throw new Error('El Excel fue leído, pero no encontré referencias con kilogramos mayores que cero.');
    ...
    res.json({
      ...
      total_referencias:Object.keys(outArticles).length,
      total_kg:Math.round(Object.values(outArticles).reduce((s,a)=>s+a.m.reduce((x,y)=>x+y,0),0))
    });
  } catch (error) {
    console.error('Error importando Excel Planeación Nogales:', error);
    res.status(400).json({ ok:false, error:error.message || 'No se pudo leer el Excel.' });
  }
});
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('✓ POST /excel/planeacion-nogales');
});
