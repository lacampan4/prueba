...
function guardarFilasEnCache(inicio, fin, filas) {
  const key = claveRango(inicio, fin);
  if (filasCache.has(key)) filasCache.delete(key);
  filasCache.set(key, { filas, dataset: null, creadoEn: Date.now() });
...
async function descargarRangoDesdeSAP(inicio, fin, actualizarEstado) {
  ...
  for (const parte of partes) {
    ...
    while (true) {
      ...
      console.log(`✓ Total acumulado en memoria: ${filas.length}`);
...
function iniciarDescarga(inicio, fin, opciones) {
  ...
  const promesa = descargarRangoDesdeSAP(inicio, fin, opciones && opciones.actualizarEstado)
    .then(filas => {
      guardarFilasEnCache(inicio, fin, filas);
      return filas;
    })
...
async function obtenerFilas(inicio, fin, { forzar = false } = {}) {
  if (!forzar) {
    ...
    if (entry) return { filas: entry.filas, deCache: true };
  }

  const filas = await iniciarDescarga(inicio, fin);
  return { filas, deCache: false };
}

async function obtenerDataset(inicio, fin, opciones) {
  const { filas, deCache } = await obtenerFilas(inicio, fin, opciones);
  const entry = filasCache.get(claveRango(inicio, fin));

  let dataset = entry && entry.dataset;
  if (!dataset) {
    dataset = buildDatasetFromFacturacion(filas);
    if (entry) entry.dataset = dataset;
  }

  return { dataset, filas, deCache };
}
...
function agruparFacturacion(filas, camposClave, filtro) {
  ...
  for (const r of filas) {
    ...
    if (!claveVals[0]) continue; // el primer campo de la clave es obligatorio (igual que "X IS NOT NULL" en SQL)
...
app.get('/sync-sap', async (req, res) => {
  ...
  if (!forzar) {
    ...
    if (entry) {
      return res.status(200).json({
        ok: true,
        yaSincronizado: true,
        mensaje: `Este rango ya se consultó hace poco (${entry.filas.length} registros, guardados en memoria de Render). No se volvió a consultar SAP.`,
        sincronizadoEn: new Date(entry.creadoEn).toISOString(),
        registros: entry.filas.length,
        fechas: { inicio, fin },
  ...
  iniciarDescarga(inicio, fin, { actualizarEstado: true })
    .then(filas => {
      ...
      console.log(`✓ Registros en memoria: ${filas.length}`);
      console.log('===========================================');
    })
    .catch(error => {
      console.error('===========================================');
...
app.get('/dataset', async (req, res) => {
  ...
  try {
    const { dataset, filas, deCache } = await obtenerDataset(fecha_inicio, fecha_fin);
...
app.get('/facturacion', async (req, res) => {
  ...
  try {
    const { filas } = await obtenerFilas(fecha_inicio, fecha_fin);
...
app.get('/dashboards/hoja-asesor', async (req, res) => {
  ...
  try {
    const { filas } = await obtenerFilas(inicio, fin);
    const grupos = agruparFacturacion(filas, ['asesor']);
...
app.get('/dashboards/hoja-cliente', async (req, res) => {
  ...
  try {
    const { filas } = await obtenerFilas(inicio, fin);
    const grupos = agruparFacturacion(filas, ['cliente', 'nit', 'ciudad']);
...
app.get('/dashboards/labor-comercial', async (req, res) => {
  ...
  try {
    const { filas } = await obtenerFilas(inicio, fin);
    const grupos = agruparFacturacion(filas, ['sede', 'asesor']);
...
app.get('/dashboards/portafolio-cartera', async (req, res) => {
  ...
  try {
    const { filas } = await obtenerFilas(inicio, fin);
    const grupos = agruparFacturacion(filas, ['grupo', 'codigo_articulo', 'articulo']);
...
app.get('/dashboards/planeacion-nogales', async (req, res) => {
  ...
  try {
    const { filas } = await obtenerFilas(inicio, fin);
    const grupos = agruparFacturacion(filas, ['sede', 'nombre_almacen', 'grupo']);
