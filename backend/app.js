...
// Estado temporal de las lecturas del dataset desde Supabase. Permite que
...
async function obtenerFilas(inicio, fin) {
  ...
  const { data, total } = await obtenerPagina(inicio, fin, 1000, 0);
  const filas = data.map(row => mapSAPRecord(row.datos_sap));
  return { filas, totalRegistros: total, filasCrudasTruncadas: total > filas.length, deCache: resultado.yaGuardado };
}

async function obtenerDataset(inicio, fin, opciones) {
  const estado = iniciarEstadoDataset(inicio, fin);
  try {
    ...
    // /dataset es SOLO lectura sobre el histórico ya guardado en Supabase.
    // No dispara sincronizaciones ni vuelve a consultar SAP.
    const resultado = { yaGuardado: true };
    actualizarEstadoDataset(inicio, fin, {
      etapa: 'leyendo_supabase',
      iniciadoLecturaEn: Date.now(),
      totalRegistros: 0,
      registrosLeidos: 0,
      registrosProcesados: 0,
      paginaActual: 0,
    ...
    // El tablero Margen usa un agregado diferente al dataset comercial.
    // Lo construimos durante la misma lectura de Supabase para que al cambiar
    // el rango también se actualice Margen, sin volver a consultar SAP.
    const margenAcc = crearAcumuladorMargen();
    const surtidosAcc = crearAcumuladorSurtidos();
    let totalRegistros = 0;
    await recorrerRegistros(inicio, fin, async (rows, offset, total, paginaInfo) => {
      const pagina = Number(paginaInfo?.pagina || 0);
      const leidosAntes = Number(offset || 0);
      actualizarEstadoDataset(inicio, fin, {
        etapa: 'leyendo_supabase',
        totalRegistros: 0,
        registrosLeidos: leidosAntes + rows.length,
    ...
    actualizarEstadoDataset(inicio, fin, {
      etapa: 'finalizando',
      registrosProcesados: totalRegistros,
      registrosLeidos: totalRegistros
    });
    ...
    return { dataset, margen, surtidos, totalRegistros, deCache: resultado.yaGuardado };
  } catch (error) {
    finalizarEstadoDataset(inicio, fin, {
      estado: 'error',
      etapa: 'error',
...
// ============================================================
// SYNC-SAP (precalienta el historial de Supabase — ya NO escribe en ninguna
...
app.get('/dataset', requiereAuth, async (req, res) => {
  ...
  try {
    const { dataset, margen, surtidos, totalRegistros, deCache } = await obtenerDataset(fecha_inicio, fecha_fin);
