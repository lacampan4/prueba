...
const MAX_FILAS_CRUDAS_CACHE = parseInt(process.env.SAP_MAX_FILAS_CRUDAS, 10) || 2000;
...
const SAP_CACHE_MINUTOS = parseFloat(process.env.SAP_CACHE_MINUTOS || '10080');

// Máximo de rangos de fechas SAP conservados en memoria.
// Se deja en 1 para evitar que varios datasets grandes se acumulen en Render.
const MAX_RANGOS_EN_CACHE = 1;
...
// ============================================================
// AVISO EN TIEMPO REAL DE "OTRO DISPOSITIVO INICIÓ SESIÓN"
// ============================================================
// Antes, el dispositivo viejo solo se enteraba de que lo desconectaron
// cuando volvía a preguntar (polling cada 1s desde el frontend), y ese
// polling se frena cuando el navegador/teléfono está inactivo. Con esto
// el servidor avisa DE INMEDIATO, en el momento en que ocurre el nuevo
// login, a la conexión (SSE) que sigue abierta del dispositivo viejo.
...
// activa (por ejemplo, se reconecta tras perder señal y en el ínterin
// entraron por otro lado), se avisa de una vez.
app.get('/auth/sesion-stream', (req, res) => {
  ...
  // Late para que Render y proxies intermedios no cierren la conexión
  // por inactividad.
...
// Las descargas se coordinan por usuario, no de forma global.
// Así una consulta de un usuario no bloquea la de otro.
const descargasGlobalesPorUsuario = new Map();

function claveUsuario(usuario) {
  return String(usuario || 'anonimo').trim().toLowerCase() || 'anonimo';
}

function claveRango(usuario, inicio, fin) {
  return `${claveUsuario(usuario)}|${inicio}|${fin}`;
}

function obtenerEntradaCache(usuario, inicio, fin) {
  const key = claveRango(usuario, inicio, fin);
...
function guardarFilasEnCache(usuario, inicio, fin, resultado) {
  const key = claveRango(usuario, inicio, fin);
...
function crearSyncState() {
  return {
    ejecutando: false, estado: 'idle', inicio: null, fin: null,
    paginaActual: 0, skipActual: 0, paginasProcesadas: 0,
    registrosSAP: 0, registrosProcesados: 0, registrosTotal: 0,
    iniciadoEn: null, terminadoEn: null, error: null, ultimaActividad: null
  };
}
function obtenerSyncState(usuario) {
  const key = claveUsuario(usuario);
  let state = syncStates.get(key);
  if (!state) { state = crearSyncState(); syncStates.set(key, state); }
  return state;
}
...
function actualizarActividad(syncState) {
...
app.get('/health', (req, res) => {
  const estados = [...syncStates.values()];
  const ejecutando = estados.find(x => x.ejecutando);
  res.json({
    ok: true,
    estado: 'ok',
    ...
    sincronizacion: {
      usuariosConSincronizacion: syncStates.size,
      ejecutando: !!ejecutando,
      estado: ejecutando ? ejecutando.estado : 'idle',
      paginaActual: ejecutando ? ejecutando.paginaActual : 0,
      paginasProcesadas: ejecutando ? ejecutando.paginasProcesadas : 0,
      registrosSAP: ejecutando ? ejecutando.registrosSAP : 0,
      registrosProcesados: ejecutando ? ejecutando.registrosProcesados : 0
    },
...
app.get('/sync-status', requiereAuth, (req, res) => {
  const syncState = obtenerSyncState(req.usuarioAuth?.sub);
  res.json({
    ok: true,
    sincronizacion: {
...
const SAP_MAX_CONCURRENCIA = Math.max(1, parseInt(process.env.SAP_MAX_CONCURRENCIA, 10) || 10);
...
async function consultarSAPPagina(inicio, fin, skip, top, syncState = null) {
  await tomarTurnoSAP();
  try {
    let intento = 0;
    while (true) {
      try {
        return await consultarSAPPaginaInterna(inicio, fin, skip, top, syncState);
      } catch (error) {
        intento++;
...
async function consultarSAPPaginaInterna(inicio, fin, skip, top, syncState = null) {
  ...
  if (syncState) actualizarActividad(syncState);
...
async function descargarRangoDesdeSAP(inicio, fin, syncState = null) {
  const actualizarEstado = !!syncState;
  ...
  for (const parte of partes) {
    ...
    while (true) {
      ...
      if (actualizarEstado) {
        ...
        actualizarActividad(syncState);
      }

      console.log('===========================================');
      console.log(`Consultando SAP ${parte.inicio} → ${parte.fin} | página ${paginaMes} | skip ${skip} | top ${SAP_PAGE_SIZE}`);

      const data = await consultarSAPPagina(parte.inicio, parte.fin, skip, SAP_PAGE_SIZE, syncState);
...
function iniciarDescarga(usuario, inicio, fin, opciones) {
  const key = claveRango(usuario, inicio, fin);
  ...
  const ejecutar = async () => {
    // Cada usuario/rango tiene su propia promesa. No se espera a una
    // descarga global de otro usuario: SAP controla la concurrencia
    // mediante SAP_MAX_CONCURRENCIA en consultarSAPPagina().
    const trabajo = descargarRangoDesdeSAP(inicio, fin, opciones && opciones.syncState ? opciones.syncState : null)
      .then(resultado => {
        guardarFilasEnCache(usuario, inicio, fin, resultado);
        return resultado;
      });

    const usuarioKey = claveUsuario(usuario);
    descargasGlobalesPorUsuario.set(usuarioKey, trabajo);
    try {
      return await trabajo;
    } finally {
      if (descargasGlobalesPorUsuario.get(usuarioKey) === trabajo) {
        descargasGlobalesPorUsuario.delete(usuarioKey);
      }
    }
  };

  const promesa = ejecutar().finally(() => descargasEnCurso.delete(key));
  descargasEnCurso.set(key, promesa);
  return promesa;
}

async function obtenerFilas(usuario, inicio, fin, { forzar = false } = {}) {
  if (!forzar) {
    const entry = obtenerEntradaCache(usuario, inicio, fin);
  ...
  const resultado = await iniciarDescarga(usuario, inicio, fin);
...
async function obtenerDataset(usuario, inicio, fin, opciones) {
...
async function obtenerAgrupado(usuario, inicio, fin, nombre, { forzar = false } = {}) {
  if (!forzar) {
    const entry = obtenerEntradaCache(usuario, inicio, fin);
...
app.get('/sync-sap', requiereAuth, async (req, res) => {
  const usuario = claveUsuario(req.usuarioAuth?.sub);
  const syncState = obtenerSyncState(usuario);
  ...
  iniciarDescarga(usuario, inicio, fin, { syncState })
    .then(resultado => {
...
app.get('/dataset', requiereAuth, async (req, res) => {
  ...
  try {
    const { dataset, totalRegistros, deCache } = await obtenerDataset(usuario, fecha_inicio, fecha_fin);
...
app.get('/facturacion', requiereAuth, async (req, res) => {
  ...
  try {
    const { filas, totalRegistros, filasCrudasTruncadas } = await obtenerFilas(usuario, fecha_inicio, fecha_fin);

    // Ordenamos la única copia de filas crudas que mantenemos en caché.
    // No crear [...filas] evita otro arreglo grande durante el pico de RAM.
...
app.get('/dashboards/hoja-asesor', requiereAuth, async (req, res) => {
  ...
  try {
    const { grupos } = await obtenerAgrupado(usuario, inicio, fin, 'hoja-asesor');
...
app.get('/dashboards/hoja-cliente', requiereAuth, async (req, res) => {
  ...
  try {
    const { grupos } = await obtenerAgrupado(usuario, inicio, fin, 'hoja-cliente');
...
app.get('/dashboards/labor-comercial', requiereAuth, async (req, res) => {
  ...
  try {
    const { grupos } = await obtenerAgrupado(usuario, inicio, fin, 'labor-comercial');
...
app.get('/dashboards/portafolio-cartera', requiereAuth, async (req, res) => {
  ...
  try {
    const { grupos } = await obtenerAgrupado(usuario, inicio, fin, 'portafolio-cartera');
...
app.get('/dashboards/planeacion-nogales', requiereAuth, async (req, res) => {
  ...
  try {
    const { grupos } = await obtenerAgrupado(usuario, inicio, fin, 'planeacion-nogales');
