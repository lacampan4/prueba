...
-- Respalda el ORDER BY fecha_factura DESC, registro_hash DESC que usa
-- /dataset y /facturacion para paginar. Sin este índice compuesto,
-- Postgres tiene que ordenar en memoria todo el conjunto filtrado en
-- cada página, lo que con cientos de miles de filas provoca
-- "statement timeout" (código 57014).
CREATE INDEX idx_sap_registros_fecha_hash
  ON public.sap_registros (fecha_factura DESC, registro_hash DESC);

CREATE INDEX idx_sap_registros_cursor_key
