...
(function () {
  ...
  // ------------------------------------------------------------
  // Antes, un solo corte de red momentáneo (wifi, Render "despertando",
  // un proxy que cierra la conexión) hacía fallar el fetch, eso se
  // convertía en un alert() y el usuario tenía que volver a darle al
  // botón. Como Render ya había terminado (o seguía) trabajando en
  // segundo plano, al reintentar salía "cargando datos guardados".
  // Ahora: cualquier fetch de esta pantalla usa fetchConReintentos, que
  // reintenta solo, en silencio, durante varios minutos antes de darse
  // por vencido. Nunca usamos alert(): los avisos se muestran en un
  // globo junto al botón que se cierra solo.
  const REINTENTO_ESPERA_BASE_MS = 2000;
  const REINTENTO_ESPERA_MAX_MS = 15000;

  async function fetchConReintentos(url, opciones, { maxIntentos = 40, onReintento } = {}) {
    let intento = 0;
    while (true) {
      intento++;
      try {
        ...
        // Error de RED (offline, DNS, conexión cerrada a mitad de
        // camino, Render arrancando de nuevo...). No es que SAP haya
        // fallado: reintentamos en vez de cortar el flujo.
  ...
  // Globo/aviso junto al botón. Se actualiza in-place y desaparece solo:
  // nunca interrumpe al usuario ni exige un clic en "Aceptar".
  function avisoSap(button, mensaje, tipo) {
    ...
    el.style.color = tipo === 'error' ? '#c0392b' : (tipo === 'ok' ? '#1f8a5b' : 'var(--muted, #667085)');
  ...
  async function getStatus() {
    const response = await fetchConReintentos(API_BASE + '/sync-status', { cache: 'no-store' }, { maxIntentos: 5 });
  ...
  async function startSync(inicio, fin) {
    ...
    const response = await fetchConReintentos(url, { method: 'GET', cache: 'no-store' }, { maxIntentos: 5 });
  ...
  async function fetchFacturacionRango(inicio, fin) {
    ...
    for (let pagina = 0; pagina < FETCH_MAX_PAGES && offset < total; pagina++) {
      ...
      const response = await fetchConReintentos(url, { cache: 'no-store' }, { maxIntentos: 20 });
    ...
    try {
      ...
      if (PAGES_QUE_USAN_DATASET.has(page) && window.lcSet) {
        ...
        const response = await fetchConReintentos(url, { cache: 'no-store' }, {
          maxIntentos: 40, // ~ hasta 8-10 min reintentando antes de avisar de un error real
          onReintento: (intento, maxIntentos) => {
            if (button) setButton(button, `SAP: cargando datos… (reintentando ${intento}/${maxIntentos})`, true);
            if (button) avisoSap(button, 'Se cortó la conexión un momento mientras se cargaban los datos. Reintentando automáticamente, no hace falta volver a darle al botón.', 'sticky');
          }
        });
        const body = await response.json().catch(() => ({}));
  ...
  async function waitForCompletion(button, inicio, fin) {
    ...
    while (true) {
      ...
      if (info.ejecutando) {
        ...
        // Tiempo estimado restante: se calcula con la velocidad real que
        // llevamos hasta ahora (registros procesados / tiempo transcurrido),
        // no con una cifra inventada. Con muy poco tiempo/avance todavía no
        // es confiable, así que no se muestra hasta tener una muestra mínima.
        const elapsedS = (Date.now() - inicioEspera) / 1000;
        const rate = elapsedS >= 5 && processed > 0 ? processed / elapsedS : 0;
        ...
        if (total > 0) {
          ...
          if (rate > 0) {
            ...
            etaTxt = ` · ${formatDuracion(restantes / rate)} restantes (estimado)`;
          }
