...
(function () {
  ...
  function buildRangePicker(button) {
    ...
    // Mantener el selector de fechas y el botón SAP agrupados al extremo derecho
    // del encabezado, dejando el botón Menú como el primer control a la izquierda.
    try {
      const tools = button.parentNode;
      if (tools && tools.style) {
        tools.style.display = 'flex';
        tools.style.alignItems = 'center';
      }
      const menuButton = document.getElementById('backMenuBtn');
      if (menuButton) {
        menuButton.style.order = '-10';
      }
      wrap.style.order = '90';
      wrap.style.marginLeft = 'auto';
      button.style.order = '91';
    } catch (e) {}
