...
(function () {
  ...
  async function getStatus() {
    ...
    const response = await fetchConReintentos(API_BASE + '/sync-status', { cache: 'no-store' }, { maxIntentos: 20, timeoutMs: 15000 });
  ...
  // Muestra el avance REAL de la lectura de Supabase mientras /dataset
  // sigue construyendo el dataset. No cambia las fechas ni la consulta:
  // únicamente consulta /dataset-status en paralelo para informar al usuario.
  function iniciarProgresoDataset(button, inicio, fin) {
    let activo = true;
    const inicioCliente = Date.now();

    const tiempoCorto = (segundos) => {
      if (!Number.isFinite(segundos) || segundos < 1) return '0 s';
    ...
    const actualizar = async () => {
      ...
      try {
        const url = API_BASE +
          '/dataset-status?fecha_inicio=' + encodeURIComponent(inicio) +
          '&fecha_fin=' + encodeURIComponent(fin);
        ...
        const body = await response.json().catch(() => ({}));
        if (!response.ok || !body.ok) throw new Error(body.error || 'No se pudo consultar el avance de la base de datos.');

        const info = body.estado || {};
        const total = Number(info.totalRegistros || 0);
        const leidos = Number(info.registrosLeidos || 0);
        const estado = info.estado || 'idle';
        const etapa = info.etapa || 'idle';
        const inicioReal = Number(info.iniciadoLecturaEn || info.iniciadoEn || inicioCliente);
        const elapsed = Math.max(0.1, (Date.now() - inicioReal) / 1000);

        if (estado === 'error') {
          avisoSap(button, '✕ Error leyendo los datos guardados: ' + (info.error || 'error desconocido'), 'error');
          return;
        }

        if (estado === 'completado') {
          const segundos = Math.max(0, (Number(info.terminadoEn || Date.now()) - inicioReal) / 1000);
          setButtonProgress(button, `BD: ✓ ${leidos.toLocaleString('es-CO')} registros · ${tiempoCorto(segundos)}`, 100, leidos, Math.max(total, leidos));
          avisoSap(button, `✓ Datos guardados cargados desde Supabase en ${tiempoCorto(segundos)}.`, 'ok');
          return;
        }

        if (!body.encontrado || etapa === 'preparando' || etapa === 'idle') {
          const segundos = (Date.now() - inicioCliente) / 1000;
          setButtonProgress(button, `BD: consultando… · ${tiempoCorto(segundos)}`, 0, 0, 0);
          avisoSap(button, `Consultando los datos guardados de Supabase para ${inicio} → ${fin}… Tiempo transcurrido: ${tiempoCorto(segundos)}.`, 'sticky');
        } else if (total > 0) {
          const pct = Math.min(100, (leidos / total) * 100);
          const rate = elapsed >= 2 && leidos > 0 ? leidos / elapsed : 0;
          const restantes = Math.max(0, total - leidos);
          const eta = rate > 0 ? restantes / rate : 0;
          const etaTxt = rate > 0 ? ` · ~${tiempoCorto(eta)} restantes` : ' · calculando tiempo…';
          setButtonProgress(button, `BD: ${leidos.toLocaleString('es-CO')}/${total.toLocaleString('es-CO')} (${pct.toFixed(0)}%)${etaTxt}`, pct, leidos, total);
          avisoSap(button, `Leyendo datos guardados desde Supabase: ${leidos.toLocaleString('es-CO')} de ${total.toLocaleString('es-CO')} registros (${pct.toFixed(0)}%). Tiempo transcurrido: ${tiempoCorto(elapsed)}${rate > 0 ? `. Tiempo estimado restante: ${tiempoCorto(eta)}.` : '.'}`, 'sticky');
        } else {
          const segundos = (Date.now() - inicioCliente) / 1000;
          setButtonProgress(button, `BD: leyendo… · ${leidos.toLocaleString('es-CO')} registros`, 0, leidos, 0);
          avisoSap(button, `Leyendo los datos guardados desde Supabase… ${leidos.toLocaleString('es-CO')} registros hasta ahora. Tiempo transcurrido: ${tiempoCorto(segundos)}.`, 'sticky');
        }
      } catch (error) {
        // Un fallo aislado de /dataset-status no debe interrumpir /dataset.
        // La consulta principal continúa; en la siguiente ronda se vuelve a intentar.
        console.warn('[Supabase] No se pudo consultar el progreso:', error);
      } finally {
        if (activo) setTimeout(actualizar, 1200);
      }
    };
    ...
    try {
      ...
      if (PAGES_QUE_USAN_DATASET.has(page) && window.lcSet) {
        ...
        const url = API_BASE +
          ...
          '&fecha_fin=' + encodeURIComponent(fin);
        detenerProgresoBD = iniciarProgresoDataset(button, inicio, fin);
        const response = await fetchConReintentos(url, { cache: 'no-store' }, {
          ...
          maxIntentos: 6,
          // Rangos de varios meses/años pueden tardar bastante en el primer
          // cálculo. Damos margen amplio para que una respuesta larga no se
          // aborte a los 45 segundos y vuelva a disparar el procesamiento.
      ...
      if (detenerProgresoBD) detenerProgresoBD();
