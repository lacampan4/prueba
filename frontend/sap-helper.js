...
(function () {
  ...
  const API_BASE = (window.LC_API_BASE || 'https://prueba-d9ro.onrender.com').replace(/\/$/, '');
  ...
  const PAGES_QUE_USAN_DATASET = new Set([
    ...
    'Hoja de Asesor.html',
    'Panorama Comercial.html',
    'Panorama del Portafolio.html',
    'Hoja de Ruta - Cliente.html',
    'Planeacion Nogales.html'
  ]);
  ...
  function init() {
    ...
    if (button.dataset.sapHelperReady === '1') return true;
    button.dataset.sapHelperReady = '1';
  ...
  // Las páginas nuevas se montan dinámicamente por el empaquetador.
  // Intentamos al cargar y observamos el DOM por si el botón aparece unos
  // milisegundos después del script.
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', init, { once: true });
  } else {
    init();
  }
  if (!document.getElementById('updateSapBtn')) {
