/* ===== Panorama del Portafolio · La Campana ===== */
...
// Rentabilidad = (venta - costo) / venta. venta = Σ(Valor Kilo × kilos), costo = Σ(Costo Kilo × kilos).
function rentCell(rev,cost){
  if(!DATA.hasRent||(!rev&&!cost))return '<span style="color:var(--txt3)">—</span>';
  const m=rev-cost,pct=rev>0?m/rev*100:0,col=pct>=20?'#1f8a5b':(pct>=8?'#d9920a':'#E10600');
  return `<span style="color:${col};font-weight:600">${nf1.format(pct)}%</span> <span style="color:var(--txt3);font-size:10px">${copM(m)}</span>`;}
...
for(const name in CLI){
  ...
  const rgByG={};(c.rg||[]).forEach(a=>{rgByG[a[0]]={rev:a[1],cost:a[2]};});
  ALL.push({name,c,kg:c.kg,monthly,cmgr:loglin(monthly),
    ...
    rev:c.rev||0,cost:c.cost||0,rgByG});
}
...
fillSel('fSuc',SUCS,'Todas');fillSel('fAse',ASES,'Todos');fillSel('fCiu',CIUS,'Todas');

// ===== state =====
const ST={suc:'',ase:'',ciu:'',tipo:'',mora:'',q:'',vol:''};
...
let sortKey='kg',sortDir=-1,cliLimit=40,riskBk=null,rentGrp='';
...
function aggregate(rows){
  const monthly=MONTHS.map(_=>0);let kg=0,cc=0,cu=0,moraVal=0,nMora=0,nCred=0,nCont=0,nMora90=0,rev=0,cost=0;
  const gAgg={},sAgg={},aAgg={},ciAgg={};
  const buckets=[[1,30,0,0],[31,60,0,0],[61,90,0,0],[91,9999,0,0]];
  rows.forEach(x=>{
    kg+=x.kg;x.monthly.forEach((v,i)=>monthly[i]+=v);rev+=x.rev||0;cost+=x.cost||0;
    if(x.cred){cc+=x.cc;cu+=x.cu;nCred++;}else nCont++;
    moraVal+=x.moraVal;if(x.nMora>0)nMora++;if(x.maxMora>90)nMora90++;
    for(const gi in x.byG){const g=gAgg[gi]=gAgg[gi]||{kg:0,m:MONTHS.map(_=>0),rev:0,cost:0};g.kg+=x.byG[gi].kg;x.byG[gi].m.forEach((v,i)=>g.m[i]+=v);const rr=x.rgByG&&x.rgByG[gi];if(rr){g.rev+=rr.rev;g.cost+=rr.cost;}}
    const s=sAgg[x.suc]=sAgg[x.suc]||{kg:0,n:0,mora:0,rev:0,cost:0};s.kg+=x.kg;s.n++;s.mora+=x.moraVal;s.rev+=x.rev||0;s.cost+=x.cost||0;
  ...
  return {n:rows.length,kg,monthly,cc,cu,moraVal,nMora,nMora90,nCred,nCont,gAgg,sAgg,aAgg,ciAgg,buckets,rev,cost};
}
...
function render(){
  ...
  document.getElementById('fCount').innerHTML=`<b>${nfK.format(A.n)}</b> clientes en vista · ${kgC(volKg)} kg · ${nfK.format(A.nCred)} crédito / ${nfK.format(A.nCont)} contado`;
  ...
  document.getElementById('trendHint').textContent=`total ${kgC(volKg)} kg · CMGR ${pCMGR!=null?nf1.format(pCMGR*100)+'%':'N/D'}`;
  ...
  const metaPctCell=(kg,meta)=>meta>0?`<span style="color:${semCol(kg/meta)}">${nf1.format(kg/meta*100)}%</span> <span style="color:var(--txt3);font-size:10px">de ${kgC(meta)} kg</span>`:`<span style="color:var(--txt3)">—</span>`;
  const sArr=Object.entries(A.sAgg).map(([k,v])=>({nm:k,kg:v.kg,n:v.n,mora:v.mora,part:v.kg/tot*100,rev:v.rev,cost:v.cost,meta:(DATA.metaSede&&DATA.metaSede[k])||0}))
    .sort((a,b)=>b.kg-a.kg);
  const sMax=sArr[0]?sArr[0].kg:1;
  document.getElementById('sucBox').innerHTML=`<div class="bhead"><div class="nm">Sucursal</div><div class="bar">Volumen</div><div class="nv">kg</div><div class="pc">Part.</div><div class="cm" style="width:64px">Clientes</div><div class="cm" style="width:96px">Rentab.</div><div class="cm" style="width:130px">Meta anual sede</div></div>`+sArr.map(s=>`<div class="barrow">
    ...
    <div class="cm" style="width:96px;font-size:11px">${rentCell(s.rev,s.cost)}</div>
    <div class="cm" style="width:130px;font-size:11px">${metaPctCell(s.kg,s.meta)}</div></div>`).join('');
  document.getElementById('sucHint').textContent=`${sArr.length} sucursales · % vs meta anual sede`;
  ...
  document.getElementById('aseBox').innerHTML=`<div class="bhead"><div class="nm" style="width:230px">Asesor</div><div class="bar">Volumen</div><div class="nv">kg</div><div class="pc">Part.</div></div>`+aArr.map(s=>`<div class="barrow">
  ...
  document.getElementById('aseHint').textContent=`top 12 de ${Object.keys(aseKgF).length}`;
  renderMeta();

  // grupos
  const gArr=Object.entries(A.gAgg).map(([gi,v])=>({gi,nm:gi>=0?CATS[gi]:'(s/g)',kg:v.kg,m:v.m,part:v.kg/tot*100,cmgr:loglin(v.m),rev:v.rev,cost:v.cost}))
    .sort((a,b)=>b.kg-a.kg);
  const gTop=gArr.slice(0,12),gMax=gTop[0]?gTop[0].kg:1;
  document.getElementById('grpBars').innerHTML=`<div class="bhead"><div class="nm">Grupo</div><div class="bar">Volumen</div><div class="nv">kg</div><div class="pc">Part.</div><div class="cm">CMGR</div><div class="cm" style="width:96px">Rentab.</div><div class="cm" style="width:84px">Meta</div></div>`+gTop.map(g=>`<div class="barrow">
    ...
    <div class="cm" style="width:96px;font-size:11px">${rentCell(g.rev,g.cost)}</div>
  ...
  document.getElementById('geoBox').innerHTML=`<div class="bhead"><div class="nm">Ciudad</div><div class="bar">Volumen</div><div class="nv">kg</div><div class="pc">Part.</div><div class="cm" style="width:96px">Clientes</div></div>`+ciArr.map(s=>`<div class="barrow">
    <div class="nm" title="${s.nm}">${s.nm}</div>
  ...
  document.getElementById('geoHint').textContent=`top 12 de ${Object.keys(A.ciAgg).length} ciudades`;
...
function renderCompare(rows,A){
  ...
  box.innerHTML=`
    ...
    <div class="gcols">
      <div class="glist"><h4><span class="dot" style="background:var(--ferrari)"></span>Grupos · mayores cambios</h4>
        <div class="ghead"><span>Grupo</span><span>${pm} → ${lm} · Cambio (%)</span></div>${gA.map(row).join('')||'<div class="foot-note">Sin datos.</div>'}</div>
      <div class="glist"><h4><span class="dot" style="background:#14161a"></span>Clientes · mayores cambios</h4>
        <div class="ghead"><span>Cliente</span><span>${pm} → ${lm} · Cambio (%)</span></div>${cA.map(row).join('')||'<div class="foot-note">Sin datos.</div>'}</div>
    </div>
...
function kpiFact(rev){
  if(!DATA.hasRent||!rev)return `<div class="kpi"><div class="accent" style="background:var(--txt3)"></div><div class="l">🧾 Facturado a la fecha</div><div class="v" style="color:var(--txt3)">—</div><div class="d">requiere columna Valor Kilo</div></div>`;
  return `<div class="kpi"><div class="accent"></div><div class="l">🧾 Facturado a la fecha</div>
    <div class="v">${copM(rev)}</div>
    <div class="d">${MES[0]}–${MES[MONTHS.length-1]} ${(MONTHS[MONTHS.length-1]||'2026').split('-')[0]} · venta acumulada</div></div>`;}
function kpiRent(rev,cost){
  if(!DATA.hasRent||(!rev&&!cost))return `<div class="kpi"><div class="accent" style="background:var(--txt3)"></div><div class="l">💰 Rentabilidad</div><div class="v" style="color:var(--txt3)">—</div><div class="d">requiere columnas Valor / Costo Kilo</div></div>`;
  const m=rev-cost,pct=rev>0?m/rev*100:0,col=pct>=20?'#1f8a5b':(pct>=8?'#d9920a':'#E10600');
  return `<div class="kpi"><div class="accent" style="background:${col}"></div><div class="l">💰 Rentabilidad</div>
    <div class="v" style="color:${col}">${nf1.format(pct)}<small>%</small></div>
    <div class="mbar"><i style="width:${Math.min(100,Math.max(0,pct)).toFixed(1)}%;background:${col}"></i></div>
    <div class="d">${copM(m)} margen · ${copM(rev)} venta</div></div>`;}
...
function renderMeta(){
  ...
  let arr=Object.keys(meta).filter(a=>meta[a]>0).map(a=>{
    ...
    const arev=(DATA.aseRev&&DATA.aseRev[a])||0,acost=(DATA.aseCost&&DATA.aseCost[a])||0;
    return {a,vend,m,pct,proj,pace,arev,acost,rent:arev>0?(arev-acost)/arev:0};
  });
  ...
  const rows=arr.map(r=>{
    ...
    return `<div class="metarow">
      ...
      <div class="meta-rent">${rentCell(r.arev,r.acost)}</div>
    </div>`;
  }).join('');
  box.innerHTML=`
    <div class="meta-sum">
    ...
    <div class="meta-colh">${[['a','Asesor'],['pct','Avance vs. meta'],['pct2','% meta'],['vend','Vendido / meta'],['proj','Proy. cierre'],['pace','Estado'],['rent','Rentab.']].map(([k,l])=>{
...
// ===== valor kilo vs. costo kilo · promedio mensual por grupo (rentabilidad) =====
function rentTrendData(){
  const RGM=DATA.rentGrupoMon||{},keys=Object.keys(RGM),n=MONTHS.length;
  const rev=new Array(n).fill(0),cost=new Array(n).fill(0),kg=new Array(n).fill(0);
  const pick=(rentGrp&&RGM[rentGrp])?[rentGrp]:keys;
  pick.forEach(g=>{const o=RGM[g];if(!o)return;for(let i=0;i<n;i++){rev[i]+=o.rev[i]||0;cost[i]+=o.cost[i]||0;kg[i]+=o.kg[i]||0;}});
  const val=rev.map((v,i)=>kg[i]>0?v/kg[i]:null),cst=cost.map((v,i)=>kg[i]>0?v/kg[i]:null);
  return {val,cst,kg};
}
function rentLineSVG(labels,valSer,cstSer){
  const allv=[];valSer.concat(cstSer).forEach(v=>{if(v!=null)allv.push(v);});
  const max=Math.max(...allv,1),min=Math.min(...allv,0),rng=(max-min)||1;
  const W=820,H=320,padL=78,padR=24,padT=22,padB=48,n=labels.length;
  const iw=W-padL-padR,ih=H-padT-padB;
  const X=i=>padL+(n<=1?iw/2:iw*i/(n-1)),Y=v=>padT+ih-((v-min)/rng)*ih;
  let yg='';for(let t=0;t<=4;t++){const v=min+rng*t/4,y=Y(v);yg+=`<line x1="${padL}" y1="${y.toFixed(1)}" x2="${W-padR}" y2="${y.toFixed(1)}" stroke="var(--line)" stroke-dasharray="2 4"/><text x="${padL-8}" y="${(y+3).toFixed(1)}" fill="var(--txt3)" font-size="9.5" text-anchor="end" font-family="IBM Plex Mono">${copM(v)}</text>`;}
  let xl='';for(let i=0;i<n;i++){const x=X(i);xl+=`<text x="${x.toFixed(1)}" y="${H-padB+18}" fill="var(--txt2)" font-size="10" text-anchor="middle" font-family="IBM Plex Mono">${labels[i]}</text>`;}
  const serie=(ser,col)=>{let d='',dots='',started=false;ser.forEach((v,i)=>{if(v==null)return;const x=X(i),y=Y(v);d+=(started?'L':'M')+x.toFixed(1)+' '+y.toFixed(1)+' ';started=true;dots+=`<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="3.2" fill="${col}"><title>${labels[i]}: ${copM(v)}/kg</title></circle>`;});return `<path d="${d}" fill="none" stroke="${col}" stroke-width="2.4" stroke-linejoin="round" stroke-linecap="round"/>${dots}`;};
  return `<svg viewBox="0 0 ${W} ${H}" style="width:100%;height:auto">${yg}<line x1="${padL}" y1="${padT+ih}" x2="${W-padR}" y2="${padT+ih}" stroke="var(--line2)"/>${serie(cstSer,'#5a6066')}${serie(valSer,'var(--ferrari)')}${xl}</svg>`;
}
function renderRentTrend(){
  const box=document.getElementById('rentTrendBox'),hint=document.getElementById('rentTrendHint'),sel=document.getElementById('rentGrpSel');
  if(!box)return;
  if(!DATA.hasRent||!DATA.rentGrupoMon||!Object.keys(DATA.rentGrupoMon).length){
    if(sel)sel.style.display='none';
    box.innerHTML=`<div class="foot-note" style="font-size:13px;padding:6px 0">Este panel se activa cuando tu Excel incluye <b>Valor Kilo</b> y <b>Costo Kilo</b>. Vuelve a cargar tu export de ventas para verlo.</div>`;
    if(hint)hint.textContent='requiere columnas Valor / Costo Kilo';return;
  }
  const {val,cst}=rentTrendData();
  box.innerHTML=rentLineSVG(MES.slice(0,MONTHS.length),val,cst)+
    `<div style="display:flex;gap:18px;font-size:11.5px;color:var(--txt2);margin-top:6px;justify-content:center">
      <span><span style="display:inline-block;width:9px;height:9px;border-radius:2px;background:var(--ferrari);margin-right:5px"></span>Valor kilo promedio</span>
      <span><span style="display:inline-block;width:9px;height:9px;border-radius:2px;background:#5a6066;margin-right:5px"></span>Costo kilo promedio</span></div>
    <div class="foot-note" style="margin-top:8px">Promedio ponderado por kilos · Valor kilo = venta del mes ÷ kg, Costo kilo = costo del mes ÷ kg. ${rentGrp?'Grupo: <b>'+rentGrp+'</b>':'Todos los grupos combinados'}.</div>`;
  const li=MONTHS.length-1,vv=val[li],cc=cst[li],mg=(vv!=null&&cc!=null&&vv>0)?(vv-cc)/vv*100:null;
  if(hint)hint.textContent=mg!=null?`margen ${MES[li]}: ${nf1.format(mg)}% · ${copM(vv-cc)}/kg`:'promedio mensual';
}
function bindRentTrend(){
  const sel=document.getElementById('rentGrpSel');if(!sel)return;
  const RGM=DATA.rentGrupoMon||{};
  if(!DATA.hasRent||!Object.keys(RGM).length){renderRentTrend();return;}
  const gk=Object.keys(RGM).sort((a,b)=>RGM[b].kg.reduce((x,y)=>x+y,0)-RGM[a].kg.reduce((x,y)=>x+y,0));
  sel.innerHTML=`<option value="">Todos los grupos</option>`+gk.map(g=>`<option value="${(g+'').replace(/"/g,'&quot;')}">${g}</option>`).join('');
  sel.value=rentGrp;
  sel.onchange=()=>{rentGrp=sel.value;renderRentTrend();};
  renderRentTrend();
}

function bindFilters(){
  const map={fSuc:'suc',fAse:'ase',fCiu:'ciu',fTipo:'tipo',fMora:'mora'};
  ...
  document.getElementById('fClear').onclick=()=>{Object.keys(ST).forEach(k=>ST[k]='');cliLimit=40;
...
function bindExcel(){
  ...
  if(btn&&modal)btn.onclick=()=>modal.classList.add('show');
...
function boot(){
  ...
  if(window.LC_DATA_IMPORTED){const fn=document.querySelector('.foot-note[style*="center"]');
  ...
  bindFilters();bindExcel();render();bindRentTrend();
