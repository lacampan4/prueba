...
(function () {
  ...
  function error401(hayToken) {
    ...
    return new Response(
      JSON.stringify({ ok: false, error: msg, mensaje: msg, sesionExpirada: true }),
  ...
  window.fetch = function (input, init) {
    ...
    return p.then(function (res) {
      if (res && res.status === 401 && url.indexOf('/auth/') === -1) return error401(!!tk);
      return res;
    });
  };
})();
...
(function(){
  ...
  function mostrarSesionExpulsada(mensaje){
    ...
    overlay.innerHTML='<div style="font:600 15px Inter,Arial,sans-serif;color:#fff;text-align:center;max-width:420px;padding:28px"><div style="font-size:42px;margin-bottom:14px">⚠</div><div style="font-size:20px;margin-bottom:10px">Sesión cerrada</div><div style="font-weight:400;line-height:1.5;margin-bottom:22px">'+String(mensaje).replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]})+'</div><button id="volverLogin" style="background:#fff;color:#c9151e;border:0;border-radius:10px;padding:12px 22px;font-weight:800;cursor:pointer">VOLVER A INICIAR SESIÓN</button></div>';
  ...
  setInterval(comprobarSesion,5000);
